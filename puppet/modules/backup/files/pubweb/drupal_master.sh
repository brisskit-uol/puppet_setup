...
REMOTE_WEB_FILES=${REMOTE_ROOT_DIR}drupal
...
#####################################################################
echo -e "\n----------------------------------------------" >> ${LOCAL_LOG_FILE}
echo "----------DRUPAL--------------------------------" >> ${LOCAL_LOG_FILE}
echo "----------------------------------------------" >> ${LOCAL_LOG_FILE}
echo "Starting drupal backup at: "`date` >> ${LOCAL_LOG_FILE}

#Do the database dump and copy first

#Where the remote backup script lives
BACKUP_SOURCE=${REMOTE_SOURCE_DIR}drupal.sh

#Run the backup script
ssh pubweb_backup@pub-drupal-2 ${BACKUP_SOURCE} >> ${LOCAL_LOG_FILE}
...
if [ ! -d "${LOCAL_FILE_DIR}drupal" ]
then
	mkdir "${LOCAL_FILE_DIR}drupal"
fi

if [ ! -d "${LOCAL_FILE_DIR}drupal/database" ]
then
        mkdir "${LOCAL_FILE_DIR}drupal/database"
fi

if [ ! -d "${LOCAL_FILE_DIR}drupal/files" ]
then
        mkdir "${LOCAL_FILE_DIR}drupal/files"
fi
...
echo "Finished drupal master at: "`date` >> ${LOCAL_LOG_FILE}
